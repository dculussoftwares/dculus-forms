...
// Type-only import: the backend's Prisma client is generated by the
// `prisma-client` generator into apps/backend/src/generated/prisma (not the
// default @prisma/client output — see apps/backend/prisma/schema.prisma).
// `import type` is fully erased at compile time, so it's safe to reference
// the ESM source directly even though this test project is CommonJS.
import type { PrismaClient, User, Organization, Form, Response, FormTemplate, FormPlugin } from '../../../apps/backend/src/generated/prisma/client.js' with { 'resolution-mode': 'import' };
