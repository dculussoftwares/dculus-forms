...
export const textInputFieldValidationSchema = baseFieldValidationSchema
        ...
        minLength: z
          ...
          .or(
            z
              ...
              .regex(
                /^\d*$/,
                'fieldSettingsConstants:errorMessages.minLengthInvalid'
              )
              .refine(
        ...
        maxLength: z
          ...
          .or(
            z
              ...
              .regex(
                /^\d*$/,
                'fieldSettingsConstants:errorMessages.maxLengthInvalid'
              )
              .refine(
                (val) => !val || parseInt(val) <= 5000,
                'fieldSettingsConstants:errorMessages.characterLimitExceeded'
  ...
  .refine(
    (data) => {
      ...
      const minLength =
        typeof data.validation.minLength === 'string'
          ? parseInt(data.validation.minLength)
  ...
  .refine(
    (data) => {
      ...
      const defaultLength = data.defaultValue.length;
      const maxLength =
        typeof data.validation.maxLength === 'string'
...
export const selectFieldValidationSchema = baseFieldValidationSchema
  ...
  .extend({
    options: z
      ...
      .min(1, 'fieldSettingsConstants:errorMessages.minOptionsRequired')
      .refine(
        (options) =>
          new Set(options.filter((opt) => opt.trim())).size ===
...
export const radioFieldValidationSchema = baseFieldValidationSchema
  ...
  .extend({
    options: z
      ...
      .min(2, 'fieldSettingsConstants:errorMessages.minRadioOptionsRequired')
      .refine(
        (options) =>
          new Set(options.filter((opt) => opt.trim())).size ===
          options.filter((opt) => opt.trim()).length,
...
export const checkboxFieldValidationSchema = baseFieldValidationSchema
  .extend({
    ...
    defaultValue: z
      .array(z.string())
      .max(100, 'fieldSettingsConstants:errorMessages.characterLimitExceeded')
      .optional()
      .or(
        z
          .string()
    ...
    validation: z
      .object({
        ...
        minSelections: z
          .number()
          .min(0, 'Minimum selections must be 0 or greater')
          .optional()
          .or(
            z
              .string()
              .regex(/^\d*$/, 'Minimum selections must be a valid number')
              .optional()
          ),
        maxSelections: z
          .number()
          .min(1, 'Maximum selections must be 1 or greater')
          .optional()
          .or(
            z
              .string()
              .regex(/^\d*$/, 'Maximum selections must be a valid number')
              .optional()
          ),
      })
      .optional()
      .refine(
        (validation) => {
          ...
          if (
            ...
            const minNum =
              typeof minSelections === 'string'
                ? parseInt(minSelections)
                : minSelections;
            const maxNum =
              typeof maxSelections === 'string'
        ...
        {
          message:
            'fieldSettingsConstants:errorMessages.minSelectionsGreaterThanMax',
          path: ['maxSelections'],
        }
      ),
  })
  .refine(
    (data) => {
      ...
      if (data.defaultValue && data.options) {
        const defaultValues = Array.isArray(data.defaultValue)
          ? data.defaultValue
          : data.defaultValue
            ? data.defaultValue
                .split(',')
        ...
        const invalidDefaults = defaultValues.filter(
          (val) => !data.options.includes(val)
        );
  ...
  .refine(
    ...
    {
      message:
        'fieldSettingsConstants:errorMessages.maxSelectionsExceedsOptions',
...
export const fileUploadFieldValidationSchema = z.object({
  label: z
    .string()
    .min(1, 'fieldSettingsConstants:errorMessages.labelRequired')
    .max(200, 'fieldSettingsConstants:errorMessages.labelTooLong'),
  ...
  maxFileSizeMb: z
    .number()
    .min(1, 'fieldSettingsConstants:errorMessages.minFileSizeInvalid')
    .max(50, 'fieldSettingsConstants:errorMessages.maxFileSizeExceeded')
    .optional(),
  maxFiles: z
    .number()
    .min(1, 'fieldSettingsConstants:errorMessages.minFilesInvalid')
    .max(10, 'fieldSettingsConstants:errorMessages.maxFilesExceeded')
    .optional(),
});
...
export const richTextFieldValidationSchema = z.object({
  content: z
    .string()
    .max(50000, 'fieldSettingsConstants:errorMessages.characterLimitExceeded')
    .default(''),
});
...
export type TextInputFieldFormData = z.infer<
  typeof textInputFieldValidationSchema
>;
export type TextAreaFieldFormData = z.infer<
  typeof textAreaFieldValidationSchema
>;
...
export type CheckboxFieldFormData = z.infer<
  typeof checkboxFieldValidationSchema
>;
