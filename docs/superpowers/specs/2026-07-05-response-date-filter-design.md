...
- `convertFiltersForExport` (`useResponsesState.ts:294-316`) only reads `filters` (per-field filters), not `submittedAtRange`/`submittedAtFilter` — confirmed by reading `handleExport` (`useResponsesState.ts:330-359`), which calls `convertFiltersForExport()` directly with no reference to the date range at all. This means **exports today already ignore the submission-date filter and the tag filter** — a pre-existing gap.
- **In scope (added after initial review):** fix `handleExport` to send the same `graphqlFilters` array already built for the table's list query (which already includes the `__submittedAt` and `__tags` scope filters) instead of the incomplete `convertFiltersForExport()` list — mirroring the exact `filters`/`filterLogic` pattern already used for the main list query in `apps/form-app/src/pages/Responses.tsx:204-205`. Confirmed via the backend resolver (`apps/backend/src/graphql/resolvers/unifiedExport.ts:70-77`) that `generateFormResponseReport` already applies whatever `filters` array it's given through the same `applyResponseFilters` function used elsewhere — **no backend change needed here either**, `convertFiltersForExport` can simply be deleted.
