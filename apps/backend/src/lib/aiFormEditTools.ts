...
export const formEditTools = {
  ...
  updateField: tool({
    ...
    }) => ({ type: 'UPDATE_FIELD' as const, ...args }),
  }),
  ...
  removeField: tool({
    ...
    execute: async (args: { fieldId: string }) => ({ type: 'REMOVE_FIELD' as const, ...args }),
  }),
  ...
  reorderFields: tool({
    ...
    execute: async (args: { pageId: string; fieldIds: string[] }) => ({
      type: 'REORDER_FIELDS' as const,
      ...args,
    }),
  }),
  ...
  updateLayout: tool({
    ...
    execute: async (args: { content?: string; customCTAButtonName?: string }) => ({
...
export type FormOperation = Awaited<ReturnType<NonNullable<(typeof formEditTools)[keyof typeof formEditTools]['execute']>>>;
