    ...
    <div className="space-y-5">
      <div className="flex items-center gap-3">
        <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ backgroundColor: '#f4faf8' }}>
          <UsersIcon className="w-5 h-5 text-[#177767]" />
        </div>
        <div>
          <h1 className="text-lg font-semibold text-[#3c323e]">Users Management</h1>
          <p className="text-xs text-[#655d67]">View and manage all users across organizations</p>
        </div>
      </div>
      ...
      {data && (
        <div className="flex items-center justify-between rounded-xl bg-white px-5 py-3.5" style={{ border: '1px solid rgba(81,76,84,0.10)' }}>
          <div>
            <p className="text-xs text-[#655d67]">Total Users</p>
            <p className="text-2xl font-light text-[#262627]">{data.adminUsers.totalCount}</p>
          </div>
      ...
      {loading && (
        <div className="flex flex-col items-center justify-center py-16">
          ...
          <p className="text-xs text-[#655d67]">Loading users…</p>
        </div>
      )}
      ...
      {error && (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-sm font-semibold mb-1 text-[#3c323e]">Error loading users</p>
          <p className="text-xs text-[#655d67]">{error.message}</p>
        </div>
      )}
      ...
      {data && data.adminUsers.totalPages > 1 && (
        <div className="flex items-center justify-center gap-3">
          ...
          <span className="text-xs text-[#655d67]">Page {currentPage} of {data.adminUsers.totalPages}</span>
