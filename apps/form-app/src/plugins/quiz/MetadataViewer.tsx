...
import type { MetadataViewerProps } from '../core/registry';

export const QuizMetadataViewer: React.FC<MetadataViewerProps> = ({ metadata }) => {
    ...
    <Card>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-3">
          ...
          {metadata.fieldResults.map((result: any, idx: number) => (
            <div
              key={result.fieldId}
              className={`border rounded-lg p-4 ${
                result.isCorrect
                  ? 'border-green-200 bg-green-50'
                  : 'border-[var(--tf-error-bg-lg)] bg-[var(--tf-error-bg)]'
              }`}
            >
