...
export const DroppablePage: React.FC<DroppablePageProps> = ({
  ...
  const getFieldCountText = (count: number) => {
    const fieldLabel =
      count === 1 ? t('fieldCount.singular') : t('fieldCount.plural');
  ...
  const isDraggingFieldFromOtherPage =
    isDraggingField &&
            ...
            <div className="flex items-center space-x-2">
              ...
              {isEditingTitle ? (
                ...
                <TypographyH3
                  ...
                  title={
                    permissions.canEditFields() && isConnected
                      ? t('editTitle')
                      : undefined
                  }
                >
                ...
                >
                  <DraggableField
                    ...
                    onUpdate={(updates: Record<string, any>) =>
                      onUpdateField(page.id, field.id, updates)
                    }
                    onRemove={() => onRemoveField(page.id, field.id)}
                    onDuplicate={() => onDuplicateField(page.id, field.id)}
                    onEdit={
                      onEditField ? () => onEditField(field.id) : undefined
                    }
                    onMoveToPage={(fieldId: string, targetPageId: string) =>
                      onMoveFieldBetweenPages(page.id, targetPageId, fieldId)
                    }
