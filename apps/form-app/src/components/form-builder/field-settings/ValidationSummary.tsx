...
export const ValidationSummary: React.FC<ValidationSummaryProps> = ({ errors }) => {
  ...
  return (
    <div 
      ...
      className="mb-6 p-4 bg-[rgba(206,93,85,0.06)] dark:bg-red-900/20 border border-[rgba(206,93,85,0.16)] dark:border-red-800 rounded-lg"
    >
      <div className="flex items-start space-x-2">
        <AlertTriangle className="w-4 h-4 text-[#ce5d55] dark:text-red-400 mt-0.5 flex-shrink-0" />
        <div className="flex-1">
          <h4 className="text-sm font-medium text-[#ce5d55] dark:text-red-200 mb-2">
            {t('title')}
          </h4>
          <ul className="space-y-1 text-sm text-red-700 dark:text-red-300">
            {globalErrors.map((err, index) => (
              <li key={index} className="flex items-start space-x-1">
                <span className="text-[#ce5d55] mt-1">•</span>
