...
export const FieldChangeCard: React.FC<FieldChangeCardProps> = ({
  ...
  const getChangeTypeColor = (changeType: ChangeType) => {
    switch (changeType) {
      ...
      case ChangeType.DELETE:
        return 'bg-[var(--tf-error-bg)] text-destructive border-[var(--tf-error-bg-lg)]';
      default:
  ...
  const getValueDisplayClass = (changeType: ChangeType, isOldValue: boolean) => {
    ...
    if (changeType === ChangeType.UPDATE) {
      return isOldValue ? 'text-destructive bg-[var(--tf-error-bg)]' : 'text-green-600 bg-green-50';
    }
    return '';
  };
  ...
  const renderValue = (value: any, fieldType: string, changeType: ChangeType, isOldValue: boolean) => {
    ...
    if (formattedValue === t('fieldChangeCard.empty')) {
      return (
        <span className={`italic text-muted-foreground ${displayClass}`}>
          {formattedValue}
        </span>
      );
    }
      ...
      <CardContent className="p-4">
        ...
        <div className="mt-3 pt-2 border-t border-gray-100">
          <div className="text-xs text-muted-foreground">
            {t('fieldChangeCard.labels.fieldId')} <code className="bg-background px-1 rounded">{fieldChange.fieldId}</code>
